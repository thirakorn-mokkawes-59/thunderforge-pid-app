#!/usr/bin/env python3
"""
PIP Equipment Symbol Extractor
Extracts P&ID PIP (Process Industry Practices) equipment symbols from JSON to individual SVG files with proper names
...
class PIPEquipmentExtractor:
    def __init__(self, json_file='pid-pip-equipment.json', svg_file='pid-pip-equipment.svg'):
        ...
        # Define PIP equipment symbol names based on typical Process Industry Practices standards
        self.predefined_names = {
            "0": "Tank - Atmospheric Storage",
            "1": "Tank - Pressurized",
            "2": "Tank - Floating Roof",
            "3": "Tank - Cone Roof",
            "4": "Vessel - Vertical",
            "5": "Vessel - Horizontal",
            "6": "Vessel - Spherical",
            "7": "Column - Distillation",
            "8": "Column - Packed",
            "9": "Column - Tray",
            "10": "Reactor - General",
            "11": "Reactor - Jacketed",
            "12": "Reactor - Fluidized Bed",
            "13": "Heat Exchanger - Shell and Tube",
            "14": "Heat Exchanger - Plate",
            "15": "Heat Exchanger - Air Cooled",
            "16": "Heat Exchanger - Double Pipe",
            "17": "Heater - Fired",
            "18": "Furnace - Box Type",
            "19": "Boiler - Package",
            "20": "Pump - Centrifugal",
            "21": "Pump - Positive Displacement",
            "22": "Pump - Reciprocating",
            "23": "Compressor - Centrifugal",
            "24": "Compressor - Reciprocating",
            "25": "Compressor - Screw",
            "26": "Blower - Centrifugal",
            "27": "Fan - Axial",
            "28": "Filter - General",
            "29": "Filter - Bag",
            "30": "Filter - Cartridge",
            "31": "Separator - Gas/Liquid",
            "32": "Separator - Cyclone",
            "33": "Separator - Centrifugal",
            "34": "Dryer - Rotary",
            "35": "Dryer - Spray",
            "36": "Mixer - Static",
            "37": "Mixer - Agitated",
    ...
    def clean_svg_content(self, svg_content):
        ...
        for line in lines:
            ...
            if is_shape:
                ...
                # For PIP equipment: determine appropriate fill
                if 'fill=' not in line and ('stroke=' in line or 'stroke-width=' in line):
                    ...
                    if 'stroke="#ff0000"' in line:
                        ...
                        # Regular equipment elements
                        if '<ellipse' in line or '<circle' in line:
                            # Circles/ellipses in equipment typically represent vessels/tanks
                            ...
                            # Rectangles in equipment can be frames or equipment bodies
                            if 'fill="#ffffff"' not in line and 'fill="#000000"' not in line:
                                # Check for size to determine if it's a main body
                                ...
                                if height_match and width_match:
                                    ...
                                    # Large rectangles are equipment bodies
                            ...
                            if 'Z' in line or 'z' in line:
                                # Closed paths for equipment bodies
                                if 'fill="#ffffff"' not in line and 'fill="#000000"' not in line:
                            ...
                            # Polygons are usually equipment shapes
        ...
        try:
            ...
            for i, symbol in enumerate(symbols):
                ...
                if title_match:
                    ...
                    if str(i) not in self.symbol_names and i < len(self.symbol_data):
                        ...
                        if 'PIP' in title or 'Equipment' in title:
            ...
            for i in range(len(self.symbol_data)):
                if str(i) not in self.symbol_names:
                    self.symbol_names[str(i)] = f"PIP Equipment {i+1}"
    ...
    def extract_symbols(self, use_names=True):
        ...
        for key, svg_content in self.symbol_data.items():
            ...
            if use_names and key in self.symbol_names:
                ...
                filename = f"pid_pip_equipment_{idx:03d}_{clean_name}.svg"
            else:
                filename = f"pid_pip_equipment_{idx:03d}.svg"
    ...
    def create_reference_csv(self):
        ...
        csv_path = 'pip_equipment_reference.csv'
    ...
    def analyze_symbols(self):
        ...
        print("\n" + "=" * 70)
        print("PIP EQUIPMENT SYMBOL ANALYSIS REPORT")
        print("=" * 70)
        ...
        print("\nPIP Equipment Types:")
        print("  • Tanks (atmospheric, pressurized, floating roof)")
        print("  • Vessels (vertical, horizontal, spherical)")
        print("  • Columns (distillation, packed, tray)")
        print("  • Reactors (general, jacketed, fluidized bed)")
        print("  • Heat Exchangers (shell & tube, plate, air cooled)")
        print("  • Pumps & Compressors")
        print("  • Separators & Filters")
        print("  • Material Handling Equipment")
    ...
    def run(self, extract_names=True, create_csv=True, analyze=True, 
        ...
        print("=" * 70)
        print("PIP EQUIPMENT SYMBOL EXTRACTOR")
        print("=" * 70)
        ...
        if create_csv and has_names:
            print(f"   Reference CSV: pip_equipment_reference.csv")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Extract PIP P&ID equipment symbols from JSON to SVG/PNG files'
    )
    parser.add_argument(
        '--json', 
        default='pid-pip-equipment.json',
        help='Input JSON file (default: pid-pip-equipment.json)'
    )
    parser.add_argument(
        '--svg', 
        default='pid-pip-equipment.svg',
        help='Original SVG file for names (default: pid-pip-equipment.svg)'
    )
    parser.add_argument(
        '--output', 
        default='svg',
    ...
    # Create extractor
    extractor = PIPEquipmentExtractor(args.json, args.svg)
