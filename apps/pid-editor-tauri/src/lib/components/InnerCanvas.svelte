...
<<<<<<< HEAD
        ...
        sourceHandle: typeof conn.from.pointIndex === 'string' 
          ? `handle-${conn.from.pointIndex}`
          : `handle-${Math.min(conn.from.pointIndex || 0, 3)}`,
  ...
  function isValidConnection(connection: any) {
    ...
    if (!isValid) {
      if (connection.source === connection.target) {
        console.log('[InnerCanvas] Rejected: self-connection not allowed by config');
      } else if (!connection.sourceHandle || !connection.targetHandle) {
        console.log('[InnerCanvas] Rejected: missing handles');
      }
    } else {
      console.log('[InnerCanvas] Connection is valid');
    }
...
<<<<<<< HEAD
    ...
    console.log('[InnerCanvas] Handle IDs:', {
      actualSourceHandle,
      actualTargetHandle,
      sourcePosition,
      targetPosition
    ...
    // Use parsed positions (could be numeric or string)
    const sourcePointIndex = sourcePosition ?? 0;
    const targetPointIndex = targetPosition ?? 0;
    
    console.log('[InnerCanvas] Parsed indices:', {
      sourcePointIndex,
      targetPointIndex
    });
=======
...
<<<<<<< HEAD
    ...
    const handleNodeInternalsUpdate = (event: CustomEvent) => {
      if (updateNodeInternals && event.detail?.nodeId) {
        console.log('[InnerCanvas] Updating node internals for:', event.detail.nodeId);
        updateNodeInternals(event.detail.nodeId);
      }
    };
